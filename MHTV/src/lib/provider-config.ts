...
export interface ProviderConfigStore {
  ...
  xtreamRegion: string;
  xtreamXmltvUrl: string;
  m3uProviderUrls: string[];
  m3uEpgUrls: string[];
  m3uRegion: string;
}
...
const defaultProviderConfig = (): ProviderConfigStore => ({
...
export function loadProviderConfig(): ProviderConfigStore {
  ...
  try {
    const raw = JSON.parse(fs.readFileSync(providerConfigPath, "utf8")) as Partial<ProviderConfigStore>;
    return {
...
export function saveProviderConfig(nextConfig: Partial<ProviderConfigStore>) {
  const current = loadProviderConfig();
  const normalized: ProviderConfigStore = {
...
export function hasXtreamProvider(config: ProviderConfigStore = loadProviderConfig()) {
  return Boolean(config.xtreamBaseUrl && config.xtreamUsername && config.xtreamPassword);
}

export function hasM3uProvider(config: ProviderConfigStore = loadProviderConfig()) {
  return config.m3uProviderUrls.length > 0;
}

export function hasKptvProvider(config: ProviderConfigStore = loadProviderConfig()) {
