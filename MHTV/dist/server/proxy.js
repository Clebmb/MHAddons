...
export async function proxyStream(req, res) {
    ...
    const headers = headersToken
        ? JSON.parse(Buffer.from(headersToken, "base64url").toString("utf8"))
        : {};
    const upstreamHeaders = Object.fromEntries(Object.entries(headers).filter(([key]) => !key.toLowerCase().startsWith("x-mhtv-")));
    if (!url) {
