...
export function inferMjhFamilyLabel(url) {
    try {
...
export function inferMjhFamilyId(url) {
    return inferMjhFamilyLabel(url).toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
}
export function listMjhFamilyOptions(urls) {
    return Array.from(new Map(urls.map((url) => {
        const id = inferMjhFamilyId(url);
        return [id, { id, label: inferMjhFamilyLabel(url) }];
    })).values());
}
export function inferTvappSourceFamily(url) {
    try {
