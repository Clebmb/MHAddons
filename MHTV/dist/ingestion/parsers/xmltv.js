...
export function parseXmltv(content) {
    const parsed = parser.parse(content);
    return arrayWrap(parsed.tv?.programme).map((programme) => {
        const titleNode = arrayWrap(programme.title)[0];
        const subtitleNode = arrayWrap(programme["sub-title"])[0];
        const descNode = arrayWrap(programme.desc)[0];
        const categoryNodes = arrayWrap(programme.category);
        return {
