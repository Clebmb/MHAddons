...
export class XtreamProvider extends BaseProvider {
    ...
    async ingest() {
        ...
        const categoryById = new Map(categories.map((category) => [String(category.category_id ?? ""), normalizeText(category.category_name ?? "Live TV")]));
        const channels = streams
            ...
            return {
                ...
                aliases: [name, stream.epg_channel_id ? String(stream.epg_channel_id) : null].filter(Boolean),
                streams: [
                    {
                        sourceId: this.id,
                        streamUrl,
        ...
        return {
            ...
            feeds: [
                ...
                ...(programmes.length ? [{ kind: "xmltv", url: xmltvUrl, region: providerConfig.xtreamRegion || "global" }] : [])
            ]
        };
    }
}
