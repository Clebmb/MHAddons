  ...
  async (
    ...
    try {
      ...
      let encodedUserData: string | undefined = z
        .string()
        .optional()
        .parse(req.headers['x-mhstreams-user-data']);
      ...
      if (!encodedUserData && !auth) {
        throw new APIError(
          ...
          undefined,
          `At least one of MHStreams-User-Data or Authorization headers must be present`
        );
      }
      ...
      const mhstreams = new MHStreams(userData);
      await mhstreams.initialise();
      const response = await mhstreams.getStreams(id, type);
      const ctx = mhstreams.getStreamContext();
