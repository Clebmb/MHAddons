...
import { createProxy, BuiltinProxyStats, BuiltinProxy } from '@mhstreams/core';
...
router.all(
  ...
  async (req: Request<ProxyParams>, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (
        (!Env.MHSTREAMS_AUTH?.has(auth.username) ||
          Env.MHSTREAMS_AUTH?.get(auth.username) !== auth.password) &&
      ...
      const connectionLimit =
        Env.MHSTREAMS_AUTH_CONNECTIONS_LIMIT?.get(auth.username) ??
        Env.MHSTREAMS_AUTH_CONNECTIONS_LIMIT?.get('*') ??
        0;
