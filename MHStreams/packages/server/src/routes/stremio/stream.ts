  ...
  async (
    ...
    const provideStreamData =
      Env.PROVIDE_STREAM_DATA !== undefined
        ...
        : (req.headers['user-agent']?.includes('MHStreams/') ?? false);

    try {
      const { type, id } = req.params;

      const mhstreams = await new MHStreams(req.userData).initialise();

      const disableAutoplay = await mhstreams.shouldStopAutoPlay(type, id);
