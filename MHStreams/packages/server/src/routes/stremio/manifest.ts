...
import { Manifest } from '@mhstreams/core';
import { createLogger } from '@mhstreams/core';
...
const manifest = async (config?: UserData): Promise<Manifest> => {
  ...
  if (config) {
    const mhstreams = new MHStreams(config, { skipFailedAddons: true });

    await mhstreams.initialise();

    catalogs = mhstreams.getCatalogs();
    resources = mhstreams.getResources();
    addonCatalogs = mhstreams.getAddonCatalogs();
  }
  return {
