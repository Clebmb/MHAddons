...
router.get(
  '/nzb/:encodedAuth/:encodedParams{/:mhstreamsAuth}/:filename.nzb',
  easynewsNzbRateLimiter,
  async (req: Request, res: Response, next: NextFunction) => {
    const {
      encodedAuth,
      encodedParams,
      mhstreamsAuth: encodedAiostreamsAuth,
      filename,
    ...
    try {
      ...
      // Parse optional MHStreams auth for bypass
      let mhstreamsAuth: { username: string; password: string } | undefined;
      if (encodedAiostreamsAuth) {
        try {
          const decoded = fromUrlSafeBase64(encodedAiostreamsAuth);
          const [username, password] = decoded.split(':');
          if (username && password) {
            mhstreamsAuth = { username, password };
          }
        } catch (e) {
          // continue without auth
          logger.debug(
            'Invalid MHStreams auth in URL, continuing without bypass'
          );
        }
      }

      // Check if Easynews NZB proxy is enabled
      if (!NzbProxyManager.isEasynewsProxyEnabled(mhstreamsAuth)) {
        res.status(503).json(
          createResponse({
            error: {
