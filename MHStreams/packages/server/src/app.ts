...
import { constants, createLogger, Env } from '@mhstreams/core';
import { StremioTransformer } from '@mhstreams/core';
...
app.get('/logo.webp', staticRateLimiter, (req, res, next) => {
  const filePath = path.resolve(
    frontendRoot,
    Env.ALTERNATE_DESIGN ? 'logo_alt.png' : 'logo.webp'
  );
    ...
    res.json({
      streams: [
        StremioTransformer.createErrorStream({
          errorDescription:
            'MHStreams v2 requires you to reconfigure. Please click this stream to reconfigure.',
