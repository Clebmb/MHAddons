...
export const getLocalStorageTemplates = (): Template[] => {
  try {
    const stored = localStorage.getItem('mhstreams-custom-templates');
    if (stored) {
...
export const getLocalStorageTemplateInputs = (
  ...
  try {
    const stored = localStorage.getItem('mhstreams-template-inputs');
    if (stored) {
...
export const saveLocalStorageTemplateInputs = (
  ...
  try {
    ...
    localStorage.setItem('mhstreams-template-inputs', JSON.stringify(all));
  } catch { }
};
