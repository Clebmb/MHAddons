...
import { Template } from '@mhstreams/core';
import {
  useTemplateLoader,
...
function Content() {
  ...
  const addonName =
    userData.addonName || status?.settings?.addonName || 'MHStreams';
  const defaultDescription = `MHStreams is a fork of AIOStreams (created by Viren070)`;
  ...
  const githubUrl = 'https://github.com/Viren070/MHStreams';
  const releasesUrl = 'https://github.com/Viren070/MHStreams/releases';
  const stremioGuideUrl =
    'https://docs.mhstreams.viren070.me/configuration/setup';
  const configGuideUrl = 'https://docs.mhstreams.viren070.me';
            ...
            <div className="text-xs md:text-sm text-[--muted] font-medium mb-2">
              {addonDescription === `MHStreams is a fork of AIOStreams (created by Viren070)` ? (
                <>
                  ...
                  <br />
                  <button onClick={donationModal.open} className="text-white italic underline hover:underline text-left">
                    Click here to donate to the original developer of AIOStreams.
                  </button>
                </>
              ) : (
                addonDescription
  ...
  const fetchReleases = React.useCallback(async (page: number = 1) => {
    try {
      const response = await fetch(
        `https://api.github.com/repos/viren070/mhstreams/releases?per_page=100&page=${page}`
      );
...
function SetupChoiceModal({
  ...
  return (
    <Modal
      ...
      title="Get Started"
      description="Choose how you'd like to set up MHStreams"
    >
