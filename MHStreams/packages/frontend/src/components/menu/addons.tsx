...
import { MergedCatalog, CatalogModification } from '@mhstreams/core';
...
import { Option, Resource } from '@mhstreams/core';
...
function SortableAddonItem({
  ...
  const getManifestUrl = (): string | undefined => {
    if (presetMetadata.ID === 'custom' || presetMetadata.ID === 'mhstreams') {
  ...
  const handleManifestUpdate = async (e: React.FormEvent) => {
    ...
    setUserData((prev) => {
      ...
      const options =
        presetMetadata.ID === 'custom' || presetMetadata.ID === 'mhstreams'
      ...
      >
        <div className="space-y-4">
          <p className="text-sm leading-relaxed">
            ...
            MHStreams. You still install and configure them from the
            community list (or using custom addon configurations),
            but they run locally on this MHStreams instance.
          </p>
          <div className="bg-[--subtle] rounded-lg p-3 space-y-2">
            ...
            <ul className="text-sm text-[--muted] space-y-1.5 list-disc list-inside">
              ...
              <li>
                Exclusive to MHStreams and can't be installed directly to
                Stremio
              </li>
            </ul>
          </div>
          <p className="text-xs text-[--muted] italic">
            Think of it like having the addon server built into MHStreams
            itself!
          </p>
        </div>
      </Modal>
...
function AddonFetchingBehaviorCard() {
  ...
  const descriptions = {
    ...
    groups:
      'Organise addons into groups with conditions. Each group can be evaluated based on results from previous groups. Read the [docs](https://docs.mhstreams.viren070.me/guides/groups) for more information.',
    dynamic:
        ...
        <TextInput
          ...
          help={
            <p>
              ...
              <a
                href="https://docs.mhstreams.viren070.me/reference/stream-expressions"
                target="_blank"
    ...
    try {
      ...
      setUserData((prev) => {
        ...
        const modifications = existingMods.map((eMod) => {
          ...
          if (eMod.id.startsWith('mhstreams.merged.')) {
            return eMod;
          }
...
function MergedCatalogsCard() {
  ...
  const allCatalogs = (userData.catalogModifications || [])
    .filter((c) => !c.id.startsWith('mhstreams.merged.')) // Exclude merged catalogs from being selected as sources
    .map((c) => ({
  ...
  const handleSave = () => {
    ...
    if (editingMergedCatalog) {
      ...
      const newId = `mhstreams.merged.${Date.now()}`;
      setUserData((prev) => ({
        ...prev,
        mergedCatalogs: [
          ...(prev.mergedCatalogs || []),
...
function SortableCatalogItem({
  ...
  const isMergedCatalog = catalog.id.startsWith('mhstreams.merged.');
