...
**AIOStreams Proxy (Newznab):** Configure the Newznab addon to proxy NZB grabs through AIOStreams by providing your `MHSTREAMS_AUTH` credentials in the addon settings (`user:password`). This makes both the query and grab come from the same AIOStreams IP.
