...
description: Step-by-step fixes for common MHSTREAMS issues.
---

## Results & Filter Issues

<Accordions>
  <Accordion title="Results appear in standalone addons but not in MHSTREAMS">
  ...
  <Accordion title="MHSTREAMS is returning results for the wrong season, episode, or movie">
  ...
  <Accordion title="Results are very slow to appear">
    MHSTREAMS waits for all configured addons to respond before returning results. If one addon is slow, everything is delayed.
...
<Accordions>
  <Accordion title="Stremio shows 'Failed to fetch' or refuses to connect">
    **Cause:** Stremio requires all addon URLs to be served over **HTTPS**. HTTP is not accepted.
    ...
    <Steps>
      <Step>
        Set up a reverse proxy in front of MHSTREAMS. Traefik and Caddy can
  ...
  <Accordion title="Configuration disappears after updating or recreating the container">
    **Cause:** No persistent volume is mounted for MHSTREAMS' data directory, so the SQLite database is lost when the container is recreated.
    ...
    <Steps>
      ...
      <Step>
        Add a volume mapping to your `compose.yaml` under the MHSTREAMS service:
  ...
  <Accordion title="TMDB API calls fail on my VPS (India / Oracle Cloud)">
    ...
    **Fix:** Route TMDB calls through WARP (or another proxy):
    ...
    2. Add `api.themoviedb.org:true` to the proxy list in your MHSTREAMS `.env`.
  ...
  <Accordion title="'Forbidden URL' error when syncing Stream Expressions">
    **Cause:** MHSTREAMS restricts which external URLs can be used for Stream Expression sync by default.
  ...
  <Accordion title="'Forbidden regex' error in filter configuration">
    **Cause:** MHSTREAMS restricts arbitrary regex patterns in filters by default.
  ...
  <Accordion title="The MHSTREAMS container starts but the configure page is blank or errors on load">
    ...
    - **`BASE_URL` mismatch** — `BASE_URL` must match the public URL your browser uses to reach MHSTREAMS (including the `https://` prefix and no trailing slash).
