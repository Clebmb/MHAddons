...
import { MHStreamsError, MHStreamsResponse } from '../main.js';
...
export class StremioTransformer {
  ...
  public showError(resource: Resource, errors: MHStreamsError[]) {
    if (
      errors.length > 0 &&
  ...
  async transformStreams(
    ...
    if (this.userData.statistics?.enabled) {
      ...
      let statisticStreams = statistics.map((statistic) => ({
        ...
        externalUrl: 'https://github.com/Viren070/MHStreams',
        streamData: {
  ...
  transformSubtitles(
    response: MHStreamsResponse<Subtitle[]>
  ): SubtitleResponse {
  ...
  transformCatalog(
    response: MHStreamsResponse<MetaPreview[]>
  ): CatalogResponse {
  ...
  async transformMeta(
    response: MHStreamsResponse<ParsedMeta | null>,
    formatterContext?: FormatterContext,
    options?: { provideStreamData?: boolean; disableAutoplay?: boolean }
  ...
  transformAddonCatalog(
    response: MHStreamsResponse<AddonCatalog[]>
  ): AddonCatalogResponse {
  ...
  static createErrorStream(options: ErrorOptions = {}): AIOStream {
    const {
      ...
      errorUrl = 'https://github.com/Viren070/MHStreams',
    } = options;
    return {
      name: errorTitle,
  ...
  static createErrorMeta(options: ErrorOptions = {}): MetaPreview {
    ...
    return {
      id: `mhstreamserror.${encodeURIComponent(JSON.stringify(options))}`,
      name: errorTitle,
      description: errorDescription,
      type: 'movie',
    };
  ...
  static createErrorAddonCatalog(options: ErrorOptions = {}): AddonCatalog {
    ...
    return {
      transportName: 'http',
      transportUrl: 'https://github.com/Viren070/MHStreams',
      manifest: {
        name: errorTitle,
        description: errorDescription,
        id: `error.${errorTitle}`,
