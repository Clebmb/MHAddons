...
export class NzbProxyManager {
   ...
   */
  static isEasynewsProxyEnabled(mhstreamsAuth?: {
    username: string;
    password: string;
  }): boolean {
    if (
      mhstreamsAuth &&
      this.isAuthorised(mhstreamsAuth.username, mhstreamsAuth.password)
    ) {
      return true;
    }
  ...
  /**
   * Check if a user has admin bypass (via MHSTREAMS_AUTH)
   */
  static isAuthorised(username: string, password: string): boolean {
    const authMap = Env.MHSTREAMS_AUTH;
   ...
   * @param mhstreamsAuth - Optional credentials to check for admin bypass
   * @returns Rate limit check result
   */
  static checkRateLimit(
    userKey: string,
    mhstreamsAuth?: { username: string; password: string }
