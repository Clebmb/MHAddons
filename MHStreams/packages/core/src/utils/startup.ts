...
const logStartupInfo = () => {
  ...
  logSection('BUILT-IN PROXY', '🔧', () => {
    if (Env.MHSTREAMS_AUTH) {
      logKeyValue('Status:', '✅ Configured');
      const users = Array.from(Env.MHSTREAMS_AUTH.keys());
      if (users.length === 0) {
        logKeyValue('Users:', '❌ None');
      } else {
        logKeyValue('Users:', '');
        for (const user of users) {
          const password = Env.MHSTREAMS_AUTH.get(user);
          const masked =
      ...
      logKeyValue(
        'Admins:',
        Env.MHSTREAMS_AUTH_ADMINS
          ? `${Env.MHSTREAMS_AUTH_ADMINS.join(', ')}`
          : '⚠️  All users'
      );
    } else {
