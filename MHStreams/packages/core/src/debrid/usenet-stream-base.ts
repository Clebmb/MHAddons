...
export abstract class UsenetStreamService implements UsenetDebridService {
  ...
  public async checkNzbs(
    ...
    // if mhstreamsAuth is present, validate it.
    if (this.auth.mhstreamsAuth) {
      try {
        BuiltinProxy.validateAuth(this.auth.mhstreamsAuth);
      } catch (error) {
        throw new DebridError('Invalid MHStreams Proxy Auth', {
          statusCode: 401,
          statusText: 'Unauthorized',
          code: 'UNAUTHORIZED',
          headers: {},
