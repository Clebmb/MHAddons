...
export class BuiltinProxy extends BaseProxy {
  public static validateAuth(auth: string): {
    ...
    if (
      !Env.MHSTREAMS_AUTH ||
      !Env.MHSTREAMS_AUTH.has(username) ||
      Env.MHSTREAMS_AUTH.get(username) !== password
    ) {
      throw new Error('Invalid credentials.');
    }
    ...
    return {
      ...
      password,
      admin:
        Env.MHSTREAMS_AUTH_ADMINS && Env.MHSTREAMS_AUTH_ADMINS.length > 0
          ? Env.MHSTREAMS_AUTH_ADMINS.includes(username)
          : true,
    };
  }
...
export class BuiltinProxyStats {
  ...
  public async getAllUserStats(): Promise<Map<string, UserStats>> {
    const users = Env.MHSTREAMS_AUTH?.keys();
