...
export class EasynewsSearchPreset extends BuiltinAddonPreset {
      ...
      },
      {
        ...
        description:
          Env.NZB_PROXY_EASYNEWS_ENABLED === false
            ? 'You must provide a valid `username:password` from `MHSTREAMS_AUTH` to use Easynews Search with all services except Easynews.'
            : 'Optionally provide a valid `username:password` `from MHSTREAMS_AUTH` to bypass NZB limits when using Easynews Search with non-Easynews services.',
        type: 'password',
        required: false,
      },
      {
  ...
  static async generateAddons(
    ...
    if (usableServices.some((s) => s.id !== constants.EASYNEWS_SERVICE))
      if (!options.mhstreamsAuth && Env.NZB_PROXY_EASYNEWS_ENABLED === false) {
        throw new Error(
          `${this.METADATA.NAME} requires the MHStreams Auth option on this instance in order to use it with the following services: ${usableServices
  ...
  protected static generateManifestUrl(
    ...
    const config = {
      ...
      mhstreamsAuth: options.mhstreamsAuth || undefined,
    };
