...
export class NekoBtStreamParser extends BuiltinStreamParser {
  ...
  // mhstreamsName maps to one of the canonical QUALITIES where possible.
  private static VIDEO_QUALITY_MAP: Array<
    { label: string; mhstreamsName?: string } | undefined
  > = [
    /* 0 */ { label: 'Other' },
    /* 1 */ { label: 'VHS' },
    /* 2 */ { label: 'LaserDisc' },
    /* 3 */ { label: 'TV - Encode', mhstreamsName: 'HDTV' },
    /* 4 */ { label: 'TV - Raw', mhstreamsName: 'HDTV' },
    /* 5 */ { label: 'DVD - Remux', mhstreamsName: 'DVDRip' },
    /* 6 */ { label: 'DVD - Encode', mhstreamsName: 'DVDRip' },
    /* 7 */ { label: 'WEB - Mini', mhstreamsName: 'WEBRip' },
    /* 8 */ { label: 'WEB - Encode', mhstreamsName: 'WEBRip' },
    /* 9 */ { label: 'WEB-DL', mhstreamsName: 'WEB-DL' },
    /*10 */ undefined,
    /*11 */ { label: 'BD - Disc', mhstreamsName: 'BluRay' },
    /*12 */ { label: 'BD - Mini', mhstreamsName: 'BluRay' },
    /*13 */ { label: 'BD - Encode', mhstreamsName: 'BluRay' },
    /*14 */ { label: 'BD - Remux', mhstreamsName: 'BluRay REMUX' },
    /*15 */ { label: 'Hybrid', mhstreamsName: 'BluRay' },
    /*16 */ { label: 'DVD - Disc', mhstreamsName: 'DVDRip' },
  ];

  private static VIDEO_CODEC_MAP: Array<{
    name: string;
    mhstreamsName?: string;
  }> = [
    { name: 'Other' },
    { name: 'H264', mhstreamsName: 'AVC' },
    { name: 'H265', mhstreamsName: 'HEVC' },
    { name: 'AV1', mhstreamsName: 'AV1' },
    { name: 'VP9' },
    { name: 'MPEG-2' },
    { name: 'MPEG-4' },
    { name: 'WMV' },
  ...
  protected getParsedFile(
    ...
    if (parsedFile && fileMetadata) {
      ...
      if (fileMetadata.codecTag && typeof fileMetadata.codecTag === 'object') {
        const aioCodec = fileMetadata.codecTag.mhstreamsName;
      ...
      if (
        ...
        const aioQuality = fileMetadata.qualityTag.mhstreamsName;
