...
export interface MHStreamsError {
  title?: string;
  description?: string;
}

export interface MHStreamsResponse<T> {
  success: boolean;
  data: T;
  errors: MHStreamsError[];
}

export interface MHStreamsOptions {
  skipFailedAddons?: boolean;
  increasedManifestTimeout?: boolean;
  bypassManifestCache?: boolean;
}

export class MHStreams {
  private userData: UserData;
  private options: MHStreamsOptions | undefined;
  ...
  constructor(userData: UserData, options?: MHStreamsOptions) {
  ...
  public async initialise(): Promise<MHStreams> {
  ...
  private checkInitialised() {
    if (!this.isInitialised) {
      throw new Error(
        'MHStreams is not initialised. Call initialise() first.'
      );
    }
  }
  ...
  public async getCatalog(
    ...
    id: string,
    extras?: string
  ): Promise<MHStreamsResponse<MetaPreview[]>> {
    logger.info(`Handling catalog request`, { type, id, extras });

    if (id.startsWith('mhstreams.merged.')) {
  ...
  public async getMeta(
    type: string,
    id: string
  ): Promise<MHStreamsResponse<ParsedMeta | null>> {
  ...
  public async getSubtitles(
    ...
    id: string,
    extras?: string
  ): Promise<MHStreamsResponse<Subtitle[]>> {
    ...
    let errors: MHStreamsError[] = this.addonInitialisationErrors.map(
      (error) => ({
  ...
  public async getAddonCatalog(
    type: string,
    id: string
  ): Promise<MHStreamsResponse<AddonCatalog[]>> {
  ...
  private async fetchResources() {
    for (const [instanceId, manifest] of Object.entries(this.manifests)) {
      ...
      for (const resource of addonResources) {
        ...
        // will not send a request to MHStreams for other id prefixes even though our other addon that didn't specify
    ...
    // if meta resouce exists, and mhstreamserror to idPrefixes only if idPrefixes is defined
    ...
    if (this.userData.catalogModifications) {
      this.finalCatalogs = this.finalCatalogs
        ...
        .filter((catalog) => {
          ...
          if (catalog.id.startsWith('mhstreams.merged.')) {
            const modification = this.userData.catalogModifications!.find(
  ...
  private validateAddon(addon: Addon) {
    ...
    if (this.userData.uuid && addon.manifestUrl.includes(this.userData.uuid)) {
      ...
      throw new Error(
        `${getAddonName(addon)} would cause infinite self scraping, ensure you wrap a different MHStreams user.`
      );
    } else if (
      ...
      throw new Error(
        `Scraping the same MHStreams instance is disabled. Please use a different MHStreams instance, or enable it through the environment variables.`
      );
    }
    if (
      addon.preset.type &&
