...
export const EasynewsSearchAddonConfigSchema = BaseDebridConfigSchema.extend({
  ...
  mhstreamsAuth: z.string().optional(), // Optional MHStreams auth for rate limit bypass
});
...
export class EasynewsSearchAddon extends BaseDebridAddon<EasynewsSearchAddonConfig> {
  ...
  constructor(config: EasynewsSearchAddonConfig, clientIp?: string) {
    ...
    // Pre-encode mhstreamsAuth if provided (for static URLs)
    if (config.mhstreamsAuth) {
      this.encodedAiostreamsAuth = Buffer.from(config.mhstreamsAuth).toString(
        'base64url'
      );
    }
  ...
  protected async _searchNzbs(parsedId: ParsedId): Promise<NZB[]> {
    // validate mhstreams auth if provided
    if (this.userData.mhstreamsAuth) {
      try {
        BuiltinProxy.validateAuth(this.userData.mhstreamsAuth);
      } catch (error) {
        throw new Error('Invalid MHStreams Auth.');
      }
    }
