...
export class EasynewsApi {
   ...
   *
   * URL format: /builtins/easynews/nzb/:encodedAuth/:encodedParams/:mhstreamsAuth?/:filename
   *
   * @param item - Search item to generate URL for
   * @param baseUrl - Base URL of the server
   * @param mhstreamsAuth - Optional MHStreams auth for admin bypass (base64url encoded "username:password")
   */
  generateNzbUrl(
    item: EasynewsSearchItem,
    baseUrl: string,
    mhstreamsAuth?: string
  ): string {
    ...
    // Build URL with optional mhstreamsAuth for bypass
    const authPart = mhstreamsAuth ? `/${mhstreamsAuth}` : '';
