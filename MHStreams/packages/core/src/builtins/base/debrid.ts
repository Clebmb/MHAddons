...
export abstract class BaseDebridAddon<T extends BaseDebridConfig> {
  ...
  public async getStreams(type: string, id: string): Promise<Stream[]> {
    ...
    if (nzbdavAuth && nzbdavAuth.mhstreamsAuth) {
    ...
    if (altmountAuth && altmountAuth.mhstreamsAuth) {
    ...
    if (nzbdavProxyIndices.length > 0 && nzbdavAuth?.mhstreamsAuth) {
      const proxy = createProxy({
        id: 'builtin',
        enabled: true,
        credentials: nzbdavAuth.mhstreamsAuth,
      });
    ...
    if (altmountProxyIndices.length > 0 && altmountAuth?.mhstreamsAuth) {
      const proxy = createProxy({
        id: 'builtin',
        enabled: true,
        credentials: altmountAuth.mhstreamsAuth,
      });
